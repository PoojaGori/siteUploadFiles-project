...
class FTPUploadHandler(FileUploadHandler):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._file._ftp_remoteDir = os.path.join(data['project'], data['asset'])


        currentDT = datetime.datetime.now()

        self._file._ftp_file_name = "%s_V(%s)_%s" %(data['process_department'],data['version'].replace(".", "_"),data['user'].replace(".", "_"))
    ...
    def new_file(self, field_name, file_name, *args, **kwargs):
        ...
        print("Uploaded File Name : {}".format(file_name))
        ...
        # self.file = UploadedFile(self.FTP_file_name, self.content_type, self._file._assumed_contentsize, self.charset, self.content_type_extra)
        # self._file = FTPFile(BytesIO(), self.FTP_file_name,self.content_type, self._file._assumed_contentsize)

        self._file._file_path = os.path.join(self._file._ftp_remoteDir, self._file._ftp_file_name)
        print('Upload attempt to %s' % (self._file._ftp_remoteDir))
        print('Uploading File : %s' % (self._file._file_path))
        # self._file._activated = True
    ...
    def append_callback(self,data):
        ...
        #per1 = '{0:.2f}'.format((self.processed_size / self._file._assumed_contentsize * 100))
        # print ("Length of chunk data : {}".format(len(data)))
        # print("Content Length : {}".format(self._file._assumed_contentsize))
    ...
    def file_complete(self, file_size):
        ...
        # new_file.file_path = self._file._file_path
